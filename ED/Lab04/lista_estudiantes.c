...
void insertar_inicio(nodo_estudiante* nuevo){
	...
	if(ref_lista->cantidad == 1){
		nuevo->ref_siguiente = nuevo; //Si es el primer nodo de la lista se apunta así mismo
	}
}
...
void insertar_final(nodo_estudiante* nuevo){
	...
	if (ref_lista->cantidad == 0){
	   insertar_inicio(nuevo);
		...
		nuevo->ref_siguiente = ref_lista->ref_inicio; //El siguiente del ultimo nodo es el primer nodo
 	}
}
