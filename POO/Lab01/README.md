...
```
    ...
    g. Los método get y set (accessor y mutator) para id, balance y tasaDeInteresAnual.  
    ...
    j. Un método llamado calcularInteresMensual() que retorne el interés mensual, calculado con la formula: balance * tasaDeInteresAnual.  
...
```
  ...
  c. Una vez el id es aceptado, el men� principal se despliega como en el ejemplo  de corrida.  
   Se puede ingresar la opci�n 1 para ver el balance actual, la 2 para retirar dinero, la 3 para depositar dinero y la 4 para salir del menú.  
...
4. Se debe incluir un archivo README que contenga el enunciado de los ejercicio
```
